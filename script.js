...
(function () {
    ...
    function setupContactForm() {
        ...
        function getFeedbackNode() {
            ...
            if (!node) {
                ...
                if (submitButton && submitButton.parentNode === form) {
                    form.insertBefore(node, submitButton.nextSibling);
                } else {
        ...
        function setFeedback(message, isError) {
            const node = getFeedbackNode();
            node.textContent = message;
            node.style.background = isError ? '#fdecea' : '#e8f5e9';
            node.style.color = isError ? '#b71c1c' : '#1b5e20';
        }

        function getSubmitEmail() {
            const encoded = form.getAttribute('data-formsubmit-user-b64') || '';
            if (!encoded) return '';
        ...
        form.addEventListener('submit', function (e) {
            ...
            const submitMode = (form.getAttribute('data-submit-mode') || '').trim();
            if (submitMode === 'formsubmit-ajax') {
                e.preventDefault();

                const submitEmail = getSubmitEmail();
                if (!submitEmail) {
                    setFeedback('No se pudo preparar el envío. Inténtalo de nuevo.', true);
                    return;
                }

                const replyTo = form.querySelector('#_replyto');
                const emailField = form.querySelector('#email');
                if (replyTo && emailField) {
                ...
                if (submitButton) {
                    ...
                    submitButton.dataset.originalText = submitButton.textContent || 'Enviar mensaje';
                ...
                fetch('https://formsubmit.co/ajax/' + submitEmail, {
                    ...
                    .then(function () {
                        setFeedback('Gracias por escribirnos. Tu mensaje fue enviado correctamente.', false);
                        form.reset();
                        updateCounter();
                    })
                    .catch(function () {
                        setFeedback('No se pudo enviar el mensaje en este momento. Inténtalo nuevamente.', true);
                    })
                    .finally(function () {
                        if (submitButton) {
            ...
            e.preventDefault();
            setFeedback('Gracias por escribirnos. Te responderemos pronto.', false);
            form.reset();
            updateCounter();
        });
    }
